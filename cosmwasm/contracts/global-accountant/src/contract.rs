...
use accountant::{
    ...
    state::{account, transfer, Modification, Kind, TokenAddress, Transfer, Reason},
    validate_transfer,
};
...
fn modify_balance(
    ...
    let mut event =
        accountant::modify_balance(deps, modification).context("failed to modify account balance")?;

    event = event.add_attribute("action", "modify_balance")
...
fn handle_vaa(mut deps: DepsMut<WormholeQuery>, info: &MessageInfo, vaa: Binary) -> anyhow::Result<Event> {
...
fn handle_governance_vaa(
    ...
    match body.payload.action {
        ...
        Action::ModifyBalance { sequence, chain_id, token_chain, token_address, kind, amount, reason } => {
            ...
            let modification = Modification { sequence, chain_id, token_chain, token_address, kind, amount, reason };
