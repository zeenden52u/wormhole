...
func runNode(cmd *cobra.Command, args []string) {
	...
	// Batch data request channel for each chain supporting batch VAAS.
	chainBatchReqC[vaa.ChainIDSolana] = make(chan *common.BatchMessageID)
	chainBatchReqC[vaa.ChainIDEthereum] = make(chan *common.BatchMessageID)
	chainBatchReqC[vaa.ChainIDTerra] = make(chan *common.BatchMessageID)
	chainBatchReqC[vaa.ChainIDTerra2] = make(chan *common.BatchMessageID)
	...
	chainBatchReqC[vaa.ChainIDAlgorand] = make(chan *common.BatchMessageID)
	if *nearRPC != "" {
		chainBatchReqC[vaa.ChainIDNear] = make(chan *common.BatchMessageID)
	}
	...
	chainBatchReqC[vaa.ChainIDPythNet] = make(chan *common.BatchMessageID)
	if *testnetMode {
		...
		chainBatchReqC[vaa.ChainIDEthereumRopsten] = make(chan *common.BatchMessageID)
		chainBatchReqC[vaa.ChainIDInjective] = make(chan *common.BatchMessageID)
	}
	...
	supervisor.New(rootCtx, logger, func(ctx context.Context) error {
		...
		if err := supervisor.Run(ctx, "ethwatch",
			ethereum.NewEthWatcher(*ethRPC, ethContractAddr, "eth", common.ReadinessEthSyncing, vaa.ChainIDEthereum, lockC, setC, 1, chainObsvReqC[vaa.ChainIDEthereum], batchC, chainBatchReqC[vaa.ChainIDEthereum], *unsafeDevMode).Run); err != nil {
			return err
		}

		if err := supervisor.Run(ctx, "bscwatch",
			ethereum.NewEthWatcher(*bscRPC, bscContractAddr, "bsc", common.ReadinessBSCSyncing, vaa.ChainIDBSC, lockC, nil, 1, chainObsvReqC[vaa.ChainIDBSC], batchC, chainBatchReqC[vaa.ChainIDBSC], *unsafeDevMode).Run); err != nil {
			return err
		}
		...
		if err := supervisor.Run(ctx, "polygonwatch",
			ethereum.NewEthWatcher(*polygonRPC, polygonContractAddr, "polygon", common.ReadinessPolygonSyncing, vaa.ChainIDPolygon, lockC, nil, polygonMinConfirmations, chainObsvReqC[vaa.ChainIDPolygon], batchC, chainBatchReqC[vaa.ChainIDPolygon], *unsafeDevMode).Run); err != nil {
		...
		if *testnetMode {
			...
			if err := supervisor.Run(ctx, "neonwatch",
				ethereum.NewEthWatcher(*neonRPC, neonContractAddr, "neon", common.ReadinessNeonSyncing, vaa.ChainIDNeon, lockC, nil, 32, chainObsvReqC[vaa.ChainIDNeon], batchC, chainBatchReqC[vaa.ChainIDNeon], *unsafeDevMode).Run); err != nil {
				return err
			}
		}
