...
import { impossible, Payload, serialiseVAA, VAA, Other } from "./vaa";
import {
  assertChain,
  ChainName,
  CHAINS,
...
function makeVAA(
  ...
  timestamp: number = 1,
  nonce: number = 1,
  sequence: bigint = BigInt(Math.floor(Math.random() * 100000000)),
  consistencyLevel = 0
): VAA<Payload | Other> {
  let v: VAA<Payload | Other> = {
    version,
    guardianSetIndex,
    signatures: [],
    timestamp,
...
let recursionHelper = yargs(hideBin(process.argv))

//Generate
recursionHelper = recursionHelper.command(
  "generate",
...
//Make VAA passthrough
recursionHelper = recursionHelper.command(
  "makevaa",
  "Create raw devnet & testnet VAAs",
  (yargs) => {
    return yargs
      .option("guardian-secret", {
        alias: "g",
        required: false,
        describe: "Guardians' secret keys (CSV), default: tilt guardian",
        type: "string",
      })
      .option("version", {
        alias: "v",
        required: false,
        describe: "VAA version, default: 1",
        type: "number",
      })
      .option("guardian-set-index", {
        alias: "i",
        required: false,
        describe: "Guardian set index, default: 0",
        type: "number",
      })
      .option("timestamp", {
        alias: "i",
        required: false,
        describe: "timestamp, default: 1",
        type: "number",
      })
      .option("nonce", {
        alias: "n",
        required: false,
        describe: "nonce, default: 1",
        type: "number",
      })
      .option("emitter-chain", {
        alias: "ec",
        required: false,
        describe: "emitter chain, default: 1",
        type: "number",
      })
      .option("emitter-address", {
        alias: "ea",
        required: false,
        describe: "emitter address in 32 byte hex format, default: 4",
        type: "string",
      })
      .option("sequence", {
        alias: "s",
        required: false,
        describe: "Sequence number tied to the emitter, default: 0",
        type: "number",
      })
      .option("consistency-level", {
        alias: "c",
        required: false,
        describe: "Consistency level, default: 0",
        type: "number",
      })
      .option("payload", {
        alias: "p",
        required: false,
        describe:
          "Payload of the string, encoded as hex. Default: empty string",
        type: "string",
      });
  },
  (argv) => {
    const content = makeVAA(
      argv["emitter-chain"] || GOVERNANCE_CHAIN,
      argv["emitter-address"] || GOVERNANCE_EMITTER,
      (argv["guardian-secret"] && argv["guardian-secret"].split(",")) || [
        TILT_SECRET,
      ],
      {type:"Other", hex:argv.payload ||"" },
      argv.version,
      argv["guardian-set-index"],
      argv.timestamp,
      argv.nonce,
      BigInt(argv.sequence || 0),
      argv["consistency-level"]
    );
    console.log(serialiseVAA(content));
  }
);
//Misc
recursionHelper = recursionHelper.command(
...
recursionHelper = recursionHelper.command("recover <digest> <signature>", "Recover an address from a signature", (yargs) => {
  return yargs
    .positional("digest", {
      describe: "digest",
      type: "string"
...
recursionHelper = recursionHelper.command("contract <network> <chain> <module>", "Print contract address", (yargs) => {
  return yargs
    .positional("network", {
      describe: "network",
      type: "string",
...
recursionHelper = recursionHelper.command("rpc <network> <chain>", "Print RPC address", (yargs) => {
  return yargs
    .positional("network", {
      describe: "network",
      type: "string",
...
//EVM utils
recursionHelper = recursionHelper.command("evm", "EVM utilites", (yargs) => {
  return yargs
    .option("rpc", {
...
const argv = recursionHelper.argv;
